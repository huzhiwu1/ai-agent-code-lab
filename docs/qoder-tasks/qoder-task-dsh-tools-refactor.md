...
**新形态：每个知识域 = 一页哲学讲解 + 1~2 个最小 AB 对比 demo**（60~150 行/个，不是 600 行）。同一场景跑两遍——朴素版（直接调函数）vs 管线版（harness 六段管线），输出对比，一眼看出设计好处。
...
### demo-01-naive.ts（朴素版，约 80~130 行）
...
### demo-02-pipeline.ts（管线版，约 150~220 行）
