...
**保留 step-1~N 渐进式格式，保留每步"机制自实现 + child 干活走真实 LLM"的铁律**，重构目标是：每步加"对照组"演示、每步拆成多文件短文件（单文件 ≤ ~150 行，入口文件更薄）。
...
- 单文件目标 ≤ 150 行，入口 ≤ 100 行；不要为了拆分而拆分（一个函数一个文件是过度设计），按"类型 / 机制 / 对照 / 演示"四个关注点切
...
- [ ] 单文件 ≤ ~150 行（入口 ≤ 100 行），没有 200+ 行的大文件
